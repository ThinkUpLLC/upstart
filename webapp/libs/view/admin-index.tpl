...
{else}
<h2>{$total_daily_signups} signup{if $total_daily_signups neq 1}s{/if}{if $total_new_subscribers > 0}, {$total_new_subscribers|number_format} conversion{if $total_new_subscribers neq 1}s{/if} ({(($total_new_subscribers*100)/$total_daily_signups)|round}%){/if}{if $total_reups > 0}, {$total_reups|number_format} re-up{if $total_reups neq 1}s{/if}{/if}{if $total_daily_refunds > 0}, {$total_daily_refunds|number_format} refund{if $total_daily_refunds neq 1}s{/if}{/if}, ${$total_daily_revenue|number_format} today.{if $reups_due > 0} {$reups_due|number_format} more re-up{if $reups_due neq 1}s{/if} due today.{/if}</h2>
