...
{else}
<h2>{$total_daily_signups} signup{if $total_daily_signups neq 1}s{/if}{if $total_reups > 0}, {$total_reups|number_format} re-up{if $total_reups neq 1}s{/if}{/if}{if $total_daily_refunds > 0}, {$total_daily_refunds|number_format} refund{if $total_daily_refunds neq 1}s{/if}{/if}, ${$total_daily_revenue|number_format} today.{if $reups_due > 0} {$reups_due|number_format} more re-up{if $reups_due neq 1}s{/if} due today.{/if}</h2>
<p>{$total_paid_subscribers|number_format} paid members: {$total_paid_subscribers_monthly|number_format} monthly, {$total_paid_subscribers_annual|number_format} annual, {$total_paid_subscribers_coupon_codes|number_format} coupon codes. {$total_active_installs|number_format} active installations.</p>
