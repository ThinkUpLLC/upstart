...
<form action="subscriber.php?action=setusername&id={$subscriber->id}" method="get"><input type="text" width="10" value="{$subscriber->subdomainified_username}" placeholder="{$subscriber->network_user_name}" name="username"> <input type="hidden" name="id" value="{$subscriber->id}"> <input type="hidden" name="action" value="setusername"><input type="submit" value="Set" class="btn btn-default"></form>
{/if}
{/if}