...
SQL;
    ...
    public function getById($id) {
        $q = 'SELECT id,full_name,email,is_admin,last_login,is_activated,password_token,' .
            ...
            'FROM tu_owners AS o WHERE id = :id';
        $vars = array(
            ':id'=>$id
        );
        if ($this->profiler_enabled) { Profiler::setDAOMethod(__METHOD__); }
    ...
    public function getAllOwners() {
        ...
        $q .= "FROM tu_owners ORDER BY last_login DESC;";
        if ($this->profiler_enabled) { Profiler::setDAOMethod(__METHOD__); }
        $ps = $this->execute($q);
    ...
    public function getAdmins() {
        ...
        $q .= "FROM tu_owners WHERE is_admin = 1 AND is_activated = 1 ORDER BY id";
        if ($this->profiler_enabled) { Profiler::setDAOMethod(__METHOD__); }
        $ps = $this->execute($q);
    ...
    public function doesOwnerExist($email) {
        $q = " SELECT email FROM tu_owners WHERE email=:email";
        $vars = array(
            ':email'=>$email
        );
        if ($this->profiler_enabled) { Profiler::setDAOMethod(__METHOD__); }
    ...
    public function getPass($email) {
        $q = "SELECT pwd FROM tu_owners  WHERE email = :email AND is_activated='1' LIMIT 1;";
        $vars = array(
            ':email'=>$email
        );
        if ($this->profiler_enabled) { Profiler::setDAOMethod(__METHOD__); }
    ...
    public function getActivationCode($email) {
        $q = " SELECT activation_code  FROM tu_owners  WHERE email=:email";
        $vars = array(
            ':email'=>$email
        );
        if ($this->profiler_enabled) { Profiler::setDAOMethod(__METHOD__); }
    ...
    private function updateActivation($email, $is_activated) {
        $q = " UPDATE tu_owners SET is_activated=:is_activated WHERE email=:email";
        $vars = array(
            ':email'=>$email,
    ...
    public function updatePassword($email, $pwd) {
        ...
        $q = " UPDATE tu_owners SET pwd=:hashed_password WHERE email=:email";
        $vars = array(
            ':email'=>$email,
    ...
    public function updateLastLogin($email) {
        $q = " UPDATE tu_owners SET last_login=now() WHERE email=:email";
        $vars = array(
            ':email'=>$email
        );
        if ($this->profiler_enabled) { Profiler::setDAOMethod(__METHOD__); }
    ...
    public function getByPasswordToken($token) {
        $q = "SELECT * FROM tu_owners WHERE password_token LIKE :token";
    ...
    public function doesAdminExist() {
        $q = "SELECT id FROM tu_owners WHERE is_admin = 1";
        if ($this->profiler_enabled) { Profiler::setDAOMethod(__METHOD__); }
        $ps = $this->execute($q);
        return $this->getDataIsReturned($ps);
    }
    ...
    public function resetAPIKey($id) {
        $q = "UPDATE tu_owners SET api_key=:api_key WHERE id=:id";
        if ($this->profiler_enabled) { Profiler::setDAOMethod(__METHOD__); }
    ...
    private function updateSalt($email, $salt) {
        $q = " UPDATE tu_owners SET pwd_salt=:salt WHERE email=:email";
        $vars = array(
            ':email'=>$email,
            ':salt'=>$salt
        );
    ...
    public function getPrivateAPIKey($email) {
        $q = "SELECT api_key_private FROM tu_owners  WHERE email = :email AND is_activated='1' LIMIT 1;";
        $vars = array(
            ':email'=>$email
        );
        if ($this->profiler_enabled) { Profiler::setDAOMethod(__METHOD__); }
