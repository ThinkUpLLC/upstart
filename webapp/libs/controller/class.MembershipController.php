    ...
    private function cancelRecurlySubscription(Subscriber $subscriber) {
        ...
        foreach ($subscriptions as $subscription) {
            if ($subscription->state == 'active') {
                ...
                $result = $subscriber_dao->setSubscriptionStatus($subscriber->id, "Refunded");
                $result = $subscriber_dao->closeAccount($subscriber->id);
                //@TODO get prorated refund and set the amount here in the call to our account closure email
                //Do we still want to send an account closure email, or do we want to let Recurly do that?
                if ($this->sendAccountClosureEmail($subscriber, 0)) {
                    return true;
                }
            }
        }
